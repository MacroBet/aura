...
export const FollowersPage: React.FC = () => {
  ...
  const fetchFollowers = async () => {
    ...
    if (error) {
      toast.error(error.message || t('failed_to_load_followers'));
      setFollowers([]);
      setLoading(false);
      return;
    }
    ...
    const targetIds = rows
      ...
      .filter((followerId: string) => followerId !== user.id);
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="sticky top-0 bg-black/95 backdrop-blur-lg border-b border-white/10 z-40 p-4">
        <h1 className="text-2xl font-bold">{t('followers')}</h1>
      </div>
      <div className="p-4">
        {loading ? (
          ...
          <p className="text-center text-gray-400 py-12">{t('empty_following_desc')}</p>
        ) : (
